defmodule CalibrationFile do
  ...
  def run(input) do
    String.split(input, "\n")
    |> Enum.map(&sum_number_extremes/1)
    |> Enum.sum()
  end

  defp sum_number_extremes(str) do
    digits =
      Regex.scan(~r/\d/, str) |> Enum.flat_map(& &1)

    digit = List.first(digits) <> List.last(digits)
...
IO.puts(CalibrationFile.run(input))
